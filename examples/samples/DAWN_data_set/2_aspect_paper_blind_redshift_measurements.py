...
map_min_R = {200: ['H1_4861A'], 250: ['O3_4959A']}
bands_vsigma = 140
...
# Declare inputs
sample_fname = './aspect_DAWN_prism_v5_flags1-2-3_selection.csv' # Table with file selection
...
# n_bins = 20
# seed = 42
# per_bin = n_target // n_bins  # 50
#
# sample_df['z_bin'] = pd.cut(sample_df.z, bins=n_bins)
#
# uniform_df = (sample_df
#               .groupby('z_bin', observed=True, group_keys=False)
#               .apply(lambda g: g.sample(min(len(g), per_bin), random_state=seed)))
#
# uniform_df = uniform_df.drop(columns='z_bin', errors='ignore')
#
# sample_df = uniform_df
...
map_min_R ={300: ['H1_4861A', 'O3_4959A']}
bands_vsigma = 140
z_min, z_max, z_step = 0.1, 15, 0.005
    ...
    for i, idx in enumerate(sample_df.index):
        ...
        if i >= 0:
            ...
            if valid:
                ...
                # Redshift fitting
                z_flux_sum, z_pixel_count = spec.fit.redshift(lines_df, band_vsigma=bands_vsigma, z_min=z_min, z_max=z_max, delta_z=z_step, mode='key', map_min_R=map_min_R)
...
output_address = 'DAWN_diagnostic_v34.png'
plot_redshift_diagnostic(lime.load_frame(sample_fname), output_address=output_address, idcs_good=idcs_good)
