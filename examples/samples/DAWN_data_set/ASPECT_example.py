...
sample_table_fname = './sample_redshift_table_v1.csv'
sample_df = lime.load_frame(sample_fname)

# Add columns to store the redshift values
if "z_aspect_pixelSum" not in sample_df.columns:
    sample_df['z_aspect_pixelSum'] = np.nan

if "z_aspect_fluxSum" not in sample_df.columns:
    sample_df['z_Key_fluxSum'] = np.nan
