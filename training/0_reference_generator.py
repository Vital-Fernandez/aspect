...
def store_line(x_arr, y_arr, class_name, i_line, synth_line, x_cord, y_cord, box_size, amp_i=np.nan, sigma_i=np.nan,
    ...
    if include_fit:
        ...
        true_flux = amp * 2.5066282746 * sigma
        true_err = 2 * 1 * np.sqrt(2 * sigma_i * 1)
...
int_ratio_range = np.logspace(np.log10(int_ratio_min), np.log10(int_ratio_max), params['int_ratio_points'])
