...
category_path = output_folder/f'pred_array_{norm}_{version}.txt'
data_matrix_path = output_folder/f'data_array_{norm}_{version}.txt'
category_arr = np.loadtxt(category_path)
...
idcs_limit = 5000
ycoords, xcoords = data_matrix[:, 0], data_matrix[:, 1]
true_flux = category_arr
sc = ax.scatter(xcoords[:idcs_limit], ycoords[:idcs_limit], c=true_flux[:idcs_limit], cmap='magma',  norm=LogNorm())
...
if flux_version is False:
    ...
    cfg['scale'] = sample_cfg['meta']['scale']
    cfg['box_size'] = sample_cfg[f'properties_{version}']['box_pixels']
    aspect.trainer.components_trainer(label, data_matrix, category_arr, cfg, None,
